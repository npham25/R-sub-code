ttScreening<- function(y=y,formula,imp.var,data,B.values=FALSE,iterations=100,sva.method=c("two-step","irw"),
	...
	for (i in 1:iterations){
		...
		repeat{
			...
			if((length(test.temp) < 1) | (sum(is.na(test.temp))>0)){
				selection[,i]<- 0;test.length<-c(test.length,0)}else{
					  selection[test.temp,i]<- 1;
					  pvalue.matrix[test.temp,i]<- test.pvalue[(test.pvalue <= test.alpha)];
					  test.length<-c(test.length,length(test.temp))}
	}		
